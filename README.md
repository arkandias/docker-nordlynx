# Docker WireGuard Container

A Docker container that provides NordVPN connectivity using WireGuard (NordLynx protocol), based
on [linuxserver/wireguard][linuxserver] image and building upon [bubuntux/nordlynx][bubuntux].
...
|     `CATEGORY`      |              | Target type for server selection. Options: `legacy_standard`, `legacy_p2p`, `legacy_double_vpn`, `legacy_obfuscated_servers`, `legacy_onion_over_vpn` ([more info][categories]). |
